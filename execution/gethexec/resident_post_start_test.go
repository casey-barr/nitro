...
func TestResidentPostStartObserverDeduplicatesSendersInFirstOccurrenceOrder(t *testing.T) {
	...
	if _, ok := store.LatestCanonical(); ok {
		t.Fatal("pending resident state became visible before canonical append")
	}
	block := types.NewBlock(header, &types.Body{}, nil, trie.NewStackTrie(nil))
	store.MarkCanonical(block)
	record, ok := store.LatestCanonical()
	if !ok {
		t.Fatal("canonical resident state missing")
	}
	if record.MessageIndex != 17 || record.MessageDigest != digest || record.TransactionCount != 3 || len(record.Senders) != 2 {
		t.Fatalf("bad record: %+v", record)
	}
	if record.Senders[0].Address != fromA || record.Senders[1].Address != fromB {
		t.Fatal("sender order was not first-occurrence order")
	}
	if record.Senders[0].Nonce != 7 || record.Senders[1].Nonce != 8 {
		t.Fatal("sender nonce snapshot mismatch")
	}
}
...
func TestResidentPostStartObserverFailsClosedOnMissingTransactionPrefix(t *testing.T) {
	...
	if observer.Error() == nil {
		t.Fatal("missing parsed transaction prefix was accepted")
	}
	if _, ok := store.LatestCanonical(); ok {
		t.Fatal("failed observer retained a record")
	}
}
...
func TestResidentObserverFailureIsCountedAndNeverPropagated(t *testing.T) {
	...
	if observer.Error() == nil {
		t.Fatal("non-authoritative prefix was accepted")
	}
	...
	if got := store.FailureCount(); got != 1 {
		t.Fatalf("failure not counted: %d", got)
	}
	if _, ok := store.LatestCanonical(); ok {
		t.Fatal("failed observer retained a record")
	}
	...
	if got := store.FailureCount(); got != 1 {
		t.Fatalf("spurious failure count: %d", got)
	}
}

func TestResidentPostStartObserverRejectsIncompleteIdentity(t *testing.T) {
	store := NewResidentPostStartStateStore()
	if _, err := store.Observer(1, common.Hash{}, types.LatestSigner(params.AllEthashProtocolChanges)); err == nil {
		t.Fatal("zero message digest accepted")
	}
	if _, err := store.Observer(1, common.HexToHash("0x1"), nil); err == nil {
...
func TestResidentObserverNilHeaderNumberLatchesInsteadOfPanicking(t *testing.T) {
	...
	if observer.Error() == nil {
		t.Fatal("nil header number was accepted")
	}
	if _, ok := store.LatestCanonical(); ok {
		t.Fatal("failed observer retained a record")
	}
...
func TestResidentStoreClearDropsRecordsOnReorg(t *testing.T) {
	...
	if _, ok := store.LatestCanonical(); !ok {
		t.Fatal("canonical record missing before reorg")
	}
	store.Clear()
	if _, ok := store.LatestCanonical(); ok {
		t.Fatal("orphaned record survived reorg clear")
	}
}
