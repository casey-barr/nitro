...
type Frame struct { LogicalSequence uint64; TransportSequence uint64; ChunkIndex uint32; ChunkCount uint32; GapEpoch uint64; Payload []byte }
const frameHeaderSize = 8 + 8*3 + 4*2 + 4 + 32 + 4
func EncodeFrame(f Frame) ([]byte, error) {
	if len(f.Payload) > 4<<20 || f.ChunkCount == 0 || f.ChunkIndex >= f.ChunkCount { return nil, errors.New("invalid frame bounds") }
	h := sha256.Sum256(f.Payload); out := make([]byte, frameHeaderSize+len(f.Payload)); copy(out[:8], frameMagic[:]); p := 8
...
func DecodeFrame(data []byte) (Frame, error) {
	if len(data) < frameHeaderSize || !bytes.Equal(data[:8], frameMagic[:]) { return Frame{}, errors.New("invalid frame header") }; p := 8; f := Frame{}
