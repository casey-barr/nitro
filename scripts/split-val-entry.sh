...
while [[ $1 == "--val-options"* ]]; do
    ...
    if [[ $1 == "--val-options-legacy" ]]; then
        setlatest=false
        echo requested legacy validator options but no legacy validator exists
        exit 1
    fi
...
# > In the nitro cmdline:
# >> edit allowed-wasm-module-roots to recognize moduleRoots of legacy validator
